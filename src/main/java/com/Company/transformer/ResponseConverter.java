...
//		companyResponse.setUsers(companyEntity.getUsers());
	...
	public AddressResponse toAddressResponse(final AddressEntity addressEntity) {
		...
		addressResponse.setRegistration(addressEntity.getRegistration());
		addressResponse.setCompany(addressEntity.getCompany());
		addressResponse.setUsers(addressEntity.getUsers());
	...
	public List<AddressResponse> toAddressResponseList (List<AddressEntity> userList){
		return userList.stream().map(p -> new AddressResponse(p.getId(),p.getCompanyId(),p.getCity(),p.getState(),p.getCountry(),p.getPincode(),p.getCompany(),p.getRegistration(),p.getUsers()))
				.collect(Collectors.toList());
	}
	...
	public List<RegistrationResponse> toRegistrationResponseList (List<RegistrationEntity> userList){
		return userList.stream().map(p -> new RegistrationResponse(p.getId(),p.getCompanyId(),p.getLegalName(),p.getRegistrationId(),p.getDateOfRegister(),p.getCompany(),p.getAddress(),p.getUsers()))
				.collect(Collectors.toList());
